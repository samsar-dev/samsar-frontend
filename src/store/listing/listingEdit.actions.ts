  ...
  try {
    ...
    const baseListing: Omit<Listing, 'details' | 'id'> & { details?: any } = {
      ...
      ...(responseData.listingAction && { 
        listingAction: responseData.listingAction === 'RENT' ? ListingAction.RENT : ListingAction.SALE 
      }),
      ...(responseData.status && { 
      ...
      ...Object.fromEntries(
        Object.entries(responseData).filter(([key]) => ![
          ...
          'images', 'details', 'createdAt', 'updatedAt', 'listingAction', 'status'
        ].includes(key))
      )
    };
    ...
    if (responseData.details && typeof responseData.details === 'object') {
      ...
      if (details.vehicles && typeof details.vehicles === 'object') {
        ...
        details.vehicles = {
          ...
          ...(vehicleDetails.enginePower && { enginePower: Number(vehicleDetails.enginePower) }),
          ...(vehicleDetails.torque && { torque: Number(vehicleDetails.torque) }),
          ...(vehicleDetails.horsepower && { horsepower: Number(vehicleDetails.horsepower) }),
          ...(vehicleDetails.operatingWeight && { operatingWeight: Number(vehicleDetails.operatingWeight) }),
          ...(vehicleDetails.payloadCapacity && { payloadCapacity: Number(vehicleDetails.payloadCapacity) }),
          ...(vehicleDetails.cargoVolume && { cargoVolume: Number(vehicleDetails.cargoVolume) })
        };
      }
    ...
    const errorMessage = error.response?.data?.message || error.message || 'Failed to update listing';
