...
const ImageEditor = lazy(
  () => import("@/components/listings/images/ImageEditor"),
);
...
const ImageManager: React.FC<ImageManagerProps> = ({
  ...
  const allImages = useMemo(() => {
    const existingUnified: UnifiedImage[] = existingImages.map(
      (url, index) => ({
        id: `existing-${index}`,
        type: "url" as const,
        src: url,
  ...
  const { state: dragState, handlers: dragHandlers } = useDragDrop(allImages, {
    ...
    onDrop: (_fromIndex: number, _toIndex: number, data: any) => {
      ...
      if (fromImage.isExisting && toImage.isExisting && onReorderExisting) {
        const existingFromIndex = existingImages.indexOf(
          fromImage.data as string,
        );
        ...
        const fromImageIndex = images.findIndex(
          (img) => URL.createObjectURL(img) === fromImage.src,
        );
        const toImageIndex = images.findIndex(
          (img) => URL.createObjectURL(img) === toImage.src,
        );
  ...
  const handleFileUpload = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        toast.success(
          t("imageManager.success.uploaded", { count: compressedFiles.length }),
        );
      } catch (error) {
      ...
      if (image.isExisting && onDeleteExisting) {
        ...
        const newImages = images.filter((_, i) => {
          const imageIndex = images.findIndex(
            (img) => URL.createObjectURL(img) === image.src,
          );
          return i !== imageIndex;
        });
        onChange(newImages);
  ...
  const handleEditSave = useCallback(
    (editedBlob: Blob) => {
      ...
      try {
        ...
        const newImages = [...images];
        const imageIndex = images.findIndex(
          (img) => URL.createObjectURL(img) === editingImage.url,
        );
        if (imageIndex !== -1) {
      ...
      >
        ...
        <p className="text-sm text-gray-500">
          {(() => {
            ...
            return text.includes("imageManager.dropzone.placeholder")
