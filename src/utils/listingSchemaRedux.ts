...
type FieldType = 'text' | 'number' | 'boolean' | 'select' | 'multiselect' | 'date' | 'textarea' | 'color' | 'featureGroup';
...
export const getInitialValuesFromSchema = (schema: ListingFieldSchema[]): Record<string, any> => {
  return schema.reduce((initialValues, field) => {
