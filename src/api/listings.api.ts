...
export const listingsAPI = {
  ...
  async deleteListing(id: string): Promise<APIResponse<Listing>> {
    try {
      ...
      const errorMsg = error.response?.data?.error || error.message || "Failed to delete listing";
  ...
  async getUserListings(params?: ListingParams): Promise<APIResponse<Listing>> {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error?.message || "Failed to fetch user listings"
      };
    }
  },
  ...
  // Create new listing
  async create(formData: FormData): Promise<APIResponse<SingleListingResponse>> {
    try {
      // Log the FormData contents before sending
      console.log('FormData entries being sent to API:');
      for (const pair of formData.entries()) {
        console.log(pair[0], ':', typeof pair[1] === 'string' ? pair[1] : 'File/Blob data');
      }
  ...
  async getTrending(limit: number = 4): Promise<APIResponse<Listing>> {
    try {
      ...
      const response = await fetch(`${API_URL}/listings/trending?${queryParams}`, {
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async getListingsByIds(ids: string[]): Promise<APIResponse<Listing>> {
    try {
      ...
      const response = await fetch(`${API_URL}/listings/by-ids?${queryParams}`, {
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async getListingsByCategory(category: ListingCategory): Promise<APIResponse<Listing>> {
    try {
      const queryParams = new URLSearchParams();
      queryParams.append("category", category);
      
      const response = await fetch(`${API_URL}/listings/by-category?${queryParams}`, {
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async getFavorites(): Promise<APIResponse<FavoritesResponse>> {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error?.message || "Failed to fetch favorites"
      };
    }
  },
